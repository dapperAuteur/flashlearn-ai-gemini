...
export const YouTubeForm = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!response.ok) throw new Error(await response.text() || 'Failed to generate flashcards from YouTube.');
    ...
    return (
        <div className="space-y-4">
            <p className="text-sm text-gray-600 dark:text-gray-400">Paste a public YouTube video link to generate flashcards from its transcript.</p>
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    <label htmlFor="youtube_url" className="sr-only">YouTube URL</label>
                    <input id="youtube_url" name="youtube_url" type="url" value={videoUrl} onChange={(e) => setVideoUrl(e.target.value)} placeholder="https://www.youtube.com/watch?v=..." className="block w-full rounded-md border-0 py-1.5 shadow-sm ring-1 ring-inset focus:ring-2 focus:ring-inset sm:text-sm sm:leading-6 bg-white dark:bg-gray-800 text-gray-900 dark:text-white ring-gray-300 dark:ring-gray-700 placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:ring-indigo-600 dark:focus:ring-indigo-500" required />
                </div>
