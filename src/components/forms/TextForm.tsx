...
export const TextForm = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!response.ok) throw new Error(await response.text() || 'Failed to generate flashcards from text file.');
            const data = await response.json();
            setFlashcards(data.flashcards);
            setFileName(data.fileName || 'Flashcards from Text File');
        } catch (err: any) {
    ...
    return (
        <div className="space-y-4">
            <p className="text-sm text-gray-600 dark:text-gray-400">Upload a text document to generate flashcards from its content.</p>
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    <label htmlFor="text_file" className="block text-sm font-medium leading-6 text-gray-900 dark:text-gray-300">Text File</label>
                    <div className="mt-2">
                        <input id="text_file" name="text_file" type="file" onChange={handleFileChange} accept=".txt,.md,.rtf" className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 dark:file:bg-indigo-900/50 file:text-indigo-700 dark:file:text-indigo-300 hover:file:bg-indigo-100 dark:hover:file:bg-indigo-900" required />
                    </div>
                    <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                        Accepted formats: TXT, MD, RTF. Max size: {FILE_SIZE_LIMIT_MB}MB.
                    </p>
                </div>
            ...
            {isLoading && <p className="text-center text-gray-500 dark:text-gray-400">Processing text file, this may take a moment...</p>}
            {error && <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-md" role="alert"><p>{error}</p></div>}
            <FlashcardResult flashcards={flashcards} initialTitle={fileName} source="Text" onSaveSuccess={() => { setFlashcards([]); setFile(null); }} />
        </div>
    );
};